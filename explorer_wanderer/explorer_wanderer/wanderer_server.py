...
class Subscriber(Node):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(LaserScan, 'scan', self.listener_callback, QoSPresetProfiles.SENSOR_DATA.value)
        self.subscription
        self.ranges = [0.0] * 180
    ...
    def listener_callback(self, msg):
        ...
        self.left_distance = msg.ranges[25]
        self.back_distance = msg.ranges[50]
        self.right_distance = msg.ranges[75]
        self.left_forward_distance = msg.ranges[12]
        self.right_forward_distance = msg.ranges[87]
...
def rotate_until_clear(subscriber, publisher, command):
    ...
    if subscriber.left_forward_distance < subscriber.right_forward_distance:  # if the robot has the wall to his left
        while subscriber.left_forward_distance < subscriber.left_distance or subscriber.forward_distance < distance_from_wall:
            rclpy.spin_once(subscriber)
            command.angular.z = -0.2 + (random()*0.03)
            publisher.publisher_.publish(command)
